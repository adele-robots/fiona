...
namespace svg
{
	...
	vector<std::string> parser::m_ids;
    ...
    //------------------------------------------------------------------------
    void parser::parse(const char* fname, bool save)
    {
    	if(save)
    		m_saveIDs = true;

    	// Si no hay que grabar los ids(solo para el neutro)
    	// y los ids ya estan grabados(para que no lo haga el background)
    	if(!m_saveIDs && m_ids.size()!=0) {
			SVG::Document doc;
			SVG::Parser parser(doc);
			parser.parse_file(fname);
			root = doc.get_root();

			// Por cada id que haya en el neutro
			int previous_level = 0;
			string previous_name;
			stack<pair<int, string> > m_stack;
			for(unsigned int i = 0; i<m_ids.size(); i++)
			{
				// Busco ese id en la imagen actual
				xmlpp::NodeSet nl = root->find("//*[@id='" +m_ids[i] + "']");
				// Si lo encuentro
				if(!nl.empty()) {
					SVG::Element* element = dynamic_cast<SVG::Element*>(nl[0]);
					string nodeName = string(element->get_name());

					//if(element->get_level() <= previous_level) {
						while(!m_stack.empty() && (element->get_level() <= m_stack.top().first || m_stack.top().second != "g")) {
							//printf("ending element %s with level %d\n", m_stack.top().second.c_str(), m_stack.top().first);
							end_element((void *)this, m_stack.top().second.c_str());
							m_stack.pop();
						}
					//}
					previous_level = element->get_level();
					previous_name = nodeName;

					//printf("adding element %s with level %d\n", nodeName.c_str(), element->get_level());
					m_stack.push(pair<int, string>(element->get_level(), nodeName));

					std::list<xmlpp::Attribute*> atributos = element->get_attributes();
					int tamano = atributos.size()*2 +1;
					const char * attributes[tamano];
					std::list<xmlpp::Attribute*>::const_iterator it;
					int j = 0;
					// vec no se utiliza para nada pero sin esto no funciona correctamente.
					// Se sobreescriben los datos de attributes sin sentido.
					vector<string> vec;
					// Almaceno los atributos en una estructura que le pasaremos a la funcion que la parsea
					for(it = atributos.begin(); it != atributos.end(); ++it){
						string nombre((*it)->get_name());
						string valor((*it)->get_value());
						attributes[j] = nombre.c_str();++j;
						attributes[j] = valor.c_str();++j;
						vec.push_back(nombre);
						vec.push_back(valor);
					}
					attributes[j] = NULL;
					// Y lo parseamos
					start_element((void *)this, nodeName.c_str(), attributes);
				}
				// Si no lo encuentro
				else {
					printf("WARNING!! id %s not found\n", m_ids[i].c_str());
				}
			}

			while(!m_stack.empty()) {
				//printf("delete element %s with level %d\n", m_stack.top().second.c_str(), m_stack.top().first);
				end_element((void *)this, m_stack.top().second.c_str());
				m_stack.pop();
			}
    	}

    	// Para el background y el neutro parseamos como antes
    	else {
    ...
    void parser::start_element(void* data, const char* el, const char** attr)
    {
        ...
        /*for(int i = 0; attr[i]; i = i + 2) {
        	if(!strcmp(attr[i], "id")) {
        		std::string id(attr[i+1]);
        ...
        {
            if(self.m_filter_flag)
            {
                throw exception("start_element: Nested filter");
            }
            self.m_path.begin_path();
            self.parse_filter(attr);
            self.m_path.end_path();
            self.m_filter_flag = true;
        }
        else
        if(self.m_filter_flag && strcmp(el, "feGaussianBlur") == 0)
        {
            self.parse_filter(el, attr);
        }
		/*else
		if(strcmp(el,"id=")==0)
		{
...
*/
    ...
    void parser::parse_path(const char** attr)
    {
        ...
        for(i = 0; attr[i]; i += 2)
        {
			...
			//MODIFICADO
			//parseriza los id de los paths
			else if(strcmp(attr[i],"id")==0)
			{
				m_path.parse_path_name_queue(attr[i+1]);
				//printf("m_saveIDs %d\n", m_saveIDs);
		        if(m_saveIDs) {
		        	m_ids.push_back(std::string(attr[i+1]));
		        	printf("Almacenado elemento con ID %s\n", attr[i+1]);
		        }
			}
			//
            else
    ...
    bool parser::parse_attr(const char* name, const char* value)
    {
        ...
        {
			if(m_saveIDs) {
				m_ids.push_back(std::string(value));
				printf("Almacenado elemento con ID %s\n", value);
			}
        }
        //else
    ...
    void parser::parse_ellipse(const char** attr)
    {
        ...
        agg::ellipse ellipse(x0,y0,rx,ry);
    ...
    void parser::parse_filter(const char** attr)
    {
        int i;
        double x = 0.0;
        double y = 0.0;
        ...
        for(i = 0; attr[i]; i += 2)
        {
            if(!parse_attr(attr[i], attr[i + 1]))
            {
                if(strcmp(attr[i], "x") == 0) x = parse_double(attr[i + 1]);
                if(strcmp(attr[i], "y") == 0) y = parse_double(attr[i + 1]);
                if(strcmp(attr[i], "width") == 0) width = parse_double(attr[i + 1]);
                if(strcmp(attr[i], "height") == 0) height = parse_double(attr[i + 1]);
            }
        }
        //TODO: create filter and do something with it
    }

    void parser::parse_filter(const char * str, const char** attr)
    {
    	if(strcmp(str, "feGaussianBlur") == 0) {
            int i;
            ...
            for(i = 0; attr[i]; i += 2)
            {
                if(!parse_attr(attr[i], attr[i + 1]))
                {
                    if(strcmp(attr[i], "stdDeviation") == 0) stdDeviation = parse_double(attr[i + 1]);
                }
            }
            //TODO: create filter and do something with it
