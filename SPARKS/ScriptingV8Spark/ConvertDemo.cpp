...
namespace cvv8 {
    ...
    {
        typedef cvv8::CtorArityDispatcher<BoundNativeCtors> Proxy;
...
v8::Handle<v8::Value> bogo_callback_function( v8::Handle<v8::Function> const & f )
{
    ...
    assert( ! f.IsEmpty() );
    return cvv8::CallForwarder<3>::Call( f, f, 3, 42.24, "hi" );
    //return f->Call( f, 0, NULL );
}

int bogo_callback2( v8::Arguments const & argv )
{
    CERR << "native this=@"<< (void const *) cvv8::CastFromJS<BoundNative>(argv.This())
...
ValueHandle BoundNative_toString( v8::Arguments const & argv )
{
    /*
      ...
      not certain.
    */
    BoundNative * f = cvv8::CastFromJS<BoundNative>(argv.This());
    return cvv8::StringBuffer() << "[object BoundNative@"<<f<<"]";
}
...
namespace cvv8 {
    ...
    struct ClassCreator_SetupBindings<BoundNative>
    {
        static void Initialize( v8::Handle<v8::Object> const & dest )
        {
            ...
            typedef cvv8::ClassCreator<BN> CC;
            CC & cc( CC::Instance() );
            if( cc.IsSealed() )
            {
...
{
    cvv8::ClassCreator<BoundNative>::Instance().SetupBindings(dest);
}
...
v8::Handle<v8::Value> bind_BoundSubNative( v8::Handle<v8::Object> dest )
{
    ...
    typedef cvv8::ClassCreator<BSN> CC;
    CC & cc( CC::Instance() );
    if( cc.IsSealed() )
    {
        cc.AddClassTo( cvv8::TypeName<BSN>::Value, dest );
        return cc.CtorFunction();
    }

    cc
        ("subFunc",
         cvv8::ConstMethodToInCa<BSN,void (),&BSN::subFunc>::Call)
        ("toString",
         cvv8::MethodToInCa<const BSN,ValueHandle (),&BSN::toString>::Call)
         //cvv8::SigToInCa<ValueHandle (BSN::*)() const,&BSN::toString>::Call)
         ("nonBoundNative",
         cvv8::MethodToInCaVoid<BSN, BSN::NonBoundType & (), &BSN::nonBoundNative>::Call)
         ("nonBoundNativeConst",
         //cvv8::ConstMethodToInCaVoid<BSN, BSN::NonBoundType const& (), &BSN::nonBoundNative>::Call
         //Equivalent:
         //cvv8::MethodToInCaVoid<const BSN, BSN::NonBoundType const& (), &BSN::nonBoundNative>::Call
         //Equivalent:
         cvv8::MethodTo<cvv8::InCaVoid, const BSN, BSN::NonBoundType const& (), &BSN::nonBoundNative>::Call
         //Error: must fail to compile OR link (likely a link error):
         //cvv8::ToInCa<BSN, BSN::NonBoundType &(), &BSN::nonBoundNative>::Call
         )
         ("puts",
         cvv8::FunctionToInCa<int (char const *),::puts>::Call)
        ;

    //typedef cvv8::ClassCreator<BoundNative> CCFoo;
...
namespace { // testing ground for some compile-time assertions...
    ...
    {
        namespace tmp = cvv8::tmp;
        namespace sl = cvv8::sl;
#define ASS ass = cvv8::tmp::Assertion
        tmp::Assertion<true> ass;
        ASS< (0 > sl::Arity< cvv8::MethodToInCa<BoundNative, int (v8::Arguments const &), &BoundNative::invoInt> >::Value)>();
        typedef CtorFwdTest CFT;
        typedef cvv8::CtorForwarder<CFT * ()> C0;
        //typedef cvv8::CtorForwarder<CFT, CtorFwdTestSub *()> C0Sub;
        typedef cvv8::CtorForwarder<CFT * (int)> C1;
        typedef cvv8::CtorForwarder<CFT * (int,int)> C2;
        typedef cvv8::CtorForwarder<CFT * (v8::Arguments const &)> Cn;
        ...
        ctor = C2::Call;
        //ctor = C0Sub::Ctor;
        typedef cvv8::Signature< CFT (C0, C1, C2) > CtorList;
        typedef cvv8::CtorArityDispatcher<CtorList> CDispatch;
        ...
        ASS<( 1 == (sl::Arity< cvv8::Signature<M1> >::Value) )>();
        ASS<( 2 == (sl::Arity< cvv8::ConstMethodToInCa<CFT, int (int,int), &CFT::bfunc> >::Value) )>();
        typedef int (CFT::*X2)(int,int) const;
        ASS<( 2 == (sl::Arity< cvv8::Signature<X2> >::Value) )>();

        ASS<( !cvv8::sl::IsConstMethod<cvv8::Signature< M1 > >::Value )>();
        ASS<( cvv8::sl::IsConstMethod<cvv8::Signature< M2 > >::Value )>();
        ASS<( 2 == (sl::Arity< cvv8::Signature< M2 > >::Value) )>();
        ASS<( !cvv8::sl::IsConstMethod<cvv8::Signature< int (int) > >::Value )>();
        ASS<( (cvv8::Signature< int (CFT::*)() const >::IsConst) )>();
        ASS<( cvv8::sl::IsConstMethod<cvv8::Signature<X2> >::Value )>();
        ASS<( !cvv8::sl::IsConstMethod<cvv8::Signature<int (int)> >::Value )>();
        ASS<( 1 == (sl::Arity< cvv8::MethodToInCa<CFT,M1,&CFT::afunc> >::Value) )>();
        ASS<( 1 == (sl::Arity< cvv8::MethodToInCa<CFT,int (int),&CFT::afunc> >::Value) )>();
        ASS<( 2 == (sl::Arity< cvv8::ConstMethodToInCa<CFT,M2,&CFT::bfunc> >::Value) )>();
        ASS<( 2 == (sl::Arity< cvv8::ConstMethodToInCa<CFT,int (int,int),&CFT::bfunc> >::Value) )>();
        //ASS<( 2 == (cvv8::ToInCa<CFT,int (int,int),&CFT::bfunc>::Arity) )>();
        ASS<( 1 == (sl::Arity< cvv8::MethodToInCa<CFT,M1,&CFT::afunc> >::Value) )>();
        ASS<( 2 == (sl::Arity< cvv8::ConstMethodToInCa<CFT,M2,&CFT::bfunc> >::Value) )>();
        ASS<( 2 == (sl::Arity< cvv8::ConstMethodToInCa<CFT,int (int,int),&CFT::bfunc> >::Value) )>();
        //ASS<( 2 == (cvv8::ToInCa<CFT,M2,&CFT::bfunc>::Arity) )>();
        //typedef cvv8::FunctionSignature<FacT> FacSig;
        typedef cvv8::FunctionSignature< CtorFwdTest * ( v8::Arguments const &  argv )> FacSig;
        ASS<( sl::Arity<FacSig>::Value < 0 )>();
        ASS<( (tmp::IsConst<cvv8::sl::At< 0, FacSig >::Type>::Value) )>();
        ASS<( (tmp::SameType< v8::Arguments const &, cvv8::sl::At< 0, cvv8::Signature<CtorFwdTest * ( v8::Arguments const &  argv )> >::Type>::Value))>();
        ASS<( (tmp::SameType< v8::Arguments const &, cvv8::sl::At< 0, FacSig >::Type>::Value))>();
        typedef cvv8::sl::At< 0, cvv8::FunctionSignature<int (int)> >::Type A0;
        ASS<( (tmp::SameType< int, A0>::Value))>();
        typedef cvv8::sl::At< 0, cvv8::FunctionToInCa<int (char const *),::puts> >::Type A1;
        ASS<( (tmp::SameType< char const *, A1>::Value))>();

        typedef cvv8::Signature< void (int, double, char const *) > CanUnlock;
        typedef cvv8::Signature< void (int, v8::Handle<v8::Value>, double) > CannotUnlock;
        typedef cvv8::Signature< void (v8::Handle<v8::Value>, double, int) > CannotUnlock2;
        ASS< 3 == cvv8::sl::Length< CannotUnlock >::Value >();
        ASS< 3 == cvv8::sl::Length< CannotUnlock2 >::Value >();
        ASS< cvv8::IsUnlockable<void>::Value >();
        ASS< cvv8::IsUnlockable<double>::Value>();
        ASS< cvv8::IsUnlockable<int>::Value>();
        ASS< !cvv8::IsUnlockable< v8::Handle<v8::Value> >::Value >();
        ASS< !cvv8::IsUnlockable< v8::Arguments >::Value >();
        ASS<cvv8::TypeListIsUnlockable<CanUnlock>::Value>();
        ASS<!cvv8::TypeListIsUnlockable<CannotUnlock2>::Value>();
        ASS<!cvv8::TypeListIsUnlockable<CannotUnlock>::Value>();
        typedef cvv8::Signature< void (v8::Handle<v8::Value>::*)(int) > CtxSig;
        ASS<!cvv8::SignatureIsUnlockable<CtxSig>::Value>();
        ASS<cvv8::TypeListIsUnlockable<CtxSig>::Value>();
#define SIU cvv8::SignatureIsUnlockable
        ASS< SIU< cvv8::Signature<int (int, double, char)> >::Value >();
        ASS< !SIU< cvv8::Signature<int (v8::Arguments)> >::Value >();
        ASS< !SIU< cvv8::Signature<int (int, v8::Arguments)> >::Value >();
        ASS< !cvv8::IsUnlockable< v8::Handle<v8::Object> >::Value >();
        ASS< !SIU< cvv8::Signature<v8::Handle<v8::Object> (int, double)> >::Value >();
        ASS< !SIU< 
                cvv8::MethodPtr<
                    BoundNative,
                    ...
                    &BoundNative::invoInt
                > >::Value >();
        ASS< !SIU< cvv8::MethodToInCa<BoundNative, int (v8::Arguments const &), &BoundNative::invoInt > >::Value>();
        

        v8::InvocationCallback cb;
        cb = cvv8::InCaLikeMethod<BoundNative, int, &BoundNative::invoInt>::Call;
        cb = cvv8::InCaLikeConstMethod<BoundNative, int, &BoundNative::invoIntConst>::Call;
        //cb = cvv8::InCaLike<BoundNative, int, &BoundNative::invoInt>::Call;
        //cb = cvv8::InCaLike<BoundNative, int, &BoundNative::invoIntConst>::Call;
        ASS< -1 == sl::Arity< cvv8::InCaLikeMethod<BoundNative, int, &BoundNative::invoInt> >::Value >();
        ASS< -1 == sl::Arity< cvv8::InCaLikeConstMethod<BoundNative, int, &BoundNative::invoIntConst> >::Value >();
        
        typedef cvv8::Signature<void (char, double, int)> TList1;
        ASS< (0 == cvv8::sl::Index<char, TList1>::Value) >();
        ASS< (1 == cvv8::sl::Index<double, TList1>::Value) >();
        ASS< (2 == cvv8::sl::Index<int, TList1>::Value) >();
        ASS< (0 > cvv8::sl::Index<uint32_t, TList1>::Value) >();
        ASS< (0 > cvv8::sl::Index<uint32_t, cvv8::Signature<void ()> >::Value) >();

        typedef cvv8::FunctionToInCa<int (char const *),::puts> PutsInCa;
        ASS< 1 == sl::Arity< PutsInCa >::Value >();
        ASS< 1 == sl::Arity< cvv8::Signature<PutsInCa::FunctionType> >::Value >();
        ASS< -1 == sl::Arity< cvv8::Signature<v8::InvocationCallback> >::Value >();
#undef SIU
...
#define ASS ass = tmp::Assertion
...

        {
            ...
            typedef cvv8::MethodSignature<T, void()> M1;
            typedef cvv8::MethodSignature<T const, void()> M2;
            typedef cvv8::ConstMethodSignature<T, void()> C1;
        ...
        {
            typedef BoundNative T;
            typedef cvv8::MethodSignature<T const, void (T::*)() const> CS0;
            typedef cvv8::MethodSignature<T const, void (T::*)()> CS02;
            typedef cvv8::MethodSignature<T const, void (T::*)(int) const> CS1;
            typedef cvv8::MethodSignature<T const, void (T::*)(int)> CS2;
