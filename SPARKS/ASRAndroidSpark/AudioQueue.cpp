...
void AudioQueue::queueAudioBuffer(uint8_t *buffer, int inputBufferSize) {

	pthread_mutex_lock(&mutex);

	while (!hasRoom(inputBufferSize)) {
		LoggerInfo("ERROR: Audio queue is full!");
...
void AudioQueue::dequeueAudioBuffer(uint8_t *buffer, int outputBufferSize) {
	pthread_mutex_lock(&mutex);


	//printf("Tamanio del buffer output: %d\n",outputBufferSize);
	//printf("Tamanio del buffer stored: %d\n",storedAudioSize);
	//LoggerInfo("Tamanio del buffer output: %d\n",outputBufferSize);
	//LoggerInfo("Tamanio del buffer stored: %d\n",storedAudioSize);
