 ...
 */
public class X509CustomFilter extends AnonymousAuthenticationFilter {
        ...
        if (certificates.length > 0) {
        	if(SecurityContextHolder.getContext().getAuthentication() == null) {
                ...
                if(!response.isCommitted()) {
                	((HttpServletResponse) response).sendRedirect("/FIOPRE/adele/ARQ_Default_Landing-flow");
    ...
    private void doAuthenticate(HttpServletRequest request, HttpServletResponse response, X509Certificate certificate) {
        ...
        try {
        	UsernamePasswordAuthenticationToken authRequest = new UsernamePasswordAuthenticationToken(certificate, getPreAuthenticatedCredentials(request));
