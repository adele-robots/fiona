...
public class SetExpressCheckoutStoreListener extends AbstractUserProcessListener {
	...
	public void procesarAjaxChangeListener(
		...
		String sparkID = HelperContext.getInstance().getValueContext("spark_id");
		// Identificador del precio seleccionado por el usuario
		String priceID = HelperContext.getInstance().getValueContext("selected_price");
		
		
		
		// Establecemos que cuando el usuario cancele vuelva al inicio de este flujo		
		String returnURL = getRequestURL().toString() + "?u=" + userID+"&s=" + sparkID + "&p=" + priceID;
		returnURL = returnURL.replaceFirst("FIOPRE_flujoInicio-flow", "FIOPRE_sparks_store-flow");
		String cancelURL = getRequestURL().toString() + "?u=" + userID + "&cancelled=true";
		cancelURL = cancelURL.replaceFirst("FIOPRE_flujoInicio-flow", "FIOPRE_sparks_store-flow");
