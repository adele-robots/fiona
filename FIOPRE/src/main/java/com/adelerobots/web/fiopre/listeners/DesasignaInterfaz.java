...
public class DesasignaInterfaz implements IProcesadorDeAjaxChangeListener {
	...
	public void procesarAjaxChangeListener(GestorEstadoComponentes gestorEstados,
		...
		// Distingo si se quiere eliminar interfaces 'provided' o 'required'
		if(interfaceType.compareTo("1")==0){			
			//Recupero las interfaces seleccionadas para eliminar
			String [] selectedInterfaces = (String [])gestorDatos.getValue(FIELD_PROVIDED_INTERFACES);			
			...
			while(itClaves.hasNext()){			
				...
				for(int i = 0; i< selectedInterfaces.length; i++){				
					if(selectedInterfaces[i].equals(value))
						providedInterfaces.remove(clave);					
				}
			}			
			...
			//Recupero las interfaces seleccionadas para eliminar
			String [] selectedInterfaces = (String [])gestorDatos.getValue(FIELD_REQUIRED_INTERFACES);			
			//Recupero las interfaces que ya hay en la lista de las proveídas del usuario
			...
			while(itClaves.hasNext()){			
				...
				for(int i = 0; i< selectedInterfaces.length; i++){				
					if(selectedInterfaces[i].equals(value))
						requiredInterfaces.remove(clave);					
				}
			}			
