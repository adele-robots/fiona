...
public class SNUpdateAvatarHostingST extends ServicioNegocio {
	
	private static FawnaLogHelper logger = FawnaLogHelper
	.getLog(SNUpdateAvatarHostingST.class);
	...
	private static final int CTE_POSICION_IDENTIFICADOR_AVATAR = 5;

	@Override
	public IContexto[] ejecutar(IContextoEjecucion contexto, IDatosEntradaTx datosEntrada) {
		if (logger.isInfoEnabled()) {
			logger.info("Inicio Ejecucion del SN030008: Actualizar avatar en BBDD de ST con nuevo hosting");
		}
		...
		String strIdentificadorAvatar = datosEntrada.getString(CTE_POSICION_IDENTIFICADOR_AVATAR);
		...
		String decryptedIdentificadorAvatar = null;
		...
		if(key != null){
			try {
				...
				if(strIdentificadorAvatar != null && !"".equals(strIdentificadorAvatar)){
					decryptedIdentificadorAvatar = EncryptionUtils.decryptBC(strIdentificadorAvatar, key);
				}
			} catch (NoSuchAlgorithmException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
		...
		String encodedIdentificadorAvatar = null;
		...
		if (pKey != null) {
			try {
				...
				if(decryptedIdentificadorAvatar != null && !"".equals(decryptedIdentificadorAvatar)){
					encodedIdentificadorAvatar = EncryptionUtils.encryptBC(decryptedIdentificadorAvatar, pKey);
				}
		...
		boolean error = false;
		try {
			error = sparkManager.soapCallUpdateAvatarHostingST(encodedEmail, encodedNumUsuarios, encodedIdUnidadTime, encodedIdResolution, encodedHighAvailability,encodedIdentificadorAvatar);
		...
		if (logger.isInfoEnabled()) {
			logger.info("Fin Ejecucion del SN030008: Actualizar avatar en BBDD de ST con nuevo hosting. Tiempo total = "
