package org.rtmpy.examples.simple.vidPublish
{
	...
	public class VidPublish extends Application
	{
        ...
        private function make_connection():void
        {
            ...
            //connection.connect(getparam('server'), getparam('username'), getparam('password')); //para gestión de rooms
            connection.connect(getparam('server'));
            trace("made connection " + getparam('server'));
        ...
        private function addVideo():void {
            ...
            vid_width = int(getparam("width", "160"));
            vid_height = int(getparam("height", "120"));
        ...
        public function NetConnectionExample(event:FlexEvent):void
        {
           ...
           private function connectStream():void {
            ...
            var record_width : int = int(getparam("recordwidth", "480"));
	        var record_height : int = int(getparam("recordheight", "320"));
	        var fps : int = int(getparam("fps", "7"));
	        var audio_rate : int = int(getparam("audiorate", "22000"));
            var bandwidth : int = int(getparam("bandwidth", "8192"));
            var quality : int = int(getparam("quality", "0"));
            ...
            if (camera != null) {
				...
				camera.setMode(record_width, record_height, fps);
